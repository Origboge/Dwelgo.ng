...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative bg-white dark:bg-[#0a0a0a] transition-colors duration-300">
      ...
      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${isScrolled || !isHomePage
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 group shrink-0">
            ...
            <span className={`text-2xl font-bold tracking-tight transition-colors ${isScrolled || !isHomePage ? 'text-zillow-600 dark:text-white' : 'text-white'
              }`}>
              Dwelgo.ng
            </span>
          </Link>
          ...
          {/* Desktop Links */}
          <div className={`hidden lg:flex items-center gap-8 ${isScrolled || !isHomePage ? 'text-slate-700 dark:text-gray-300' : 'text-white'
            }`}>
            {navLinks.map((link) => (
              <Link
                key={link.name}
          ...
          <div className="hidden md:flex items-center gap-4 shrink-0">
            <button
              onClick={toggleTheme}
              className={`transition-colors hover:text-zillow-600 ${isScrolled || !isHomePage ? 'text-slate-700 dark:text-gray-300' : 'text-white'
                }`}
            >
            ...
            {user ? (
              <div className="flex items-center gap-3 pl-2">
                ...
                <Link to="/dashboard">
                  <img
                    ...
                    alt={user.firstName}
                    className="w-9 h-9 rounded-full border border-gray-200 dark:border-gray-700 object-cover cursor-pointer hover:opacity-80 transition-opacity"
                  />
                </Link>
                <Button
                  variant="ghost"
                  size="sm"
                  className={isScrolled || !isHomePage ? 'text-slate-700' : 'text-white hover:bg-white/20'}
                  onClick={async () => {
                    await logout();
                    navigate('/');
                  }}
              ...
              <div className="flex items-center gap-3 pl-2">
                <Link to="/login" className={`text-sm font-bold hover:text-zillow-400 transition-colors ${isScrolled || !isHomePage ? 'text-zillow-600 dark:text-white' : 'text-white'
                  }`}>Sign In</Link>
                <Link to="/register">
                  <Button variant={isScrolled || !isHomePage ? 'primary' : 'glass'} size="sm" className="rounded-md">Get Started</Button>
                </Link>
              </div>
            )}
          </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-[60]">
          ...
          {/* Drawer */}
          <div className="absolute top-0 right-0 h-full w-[80%] max-w-sm bg-white dark:bg-slate-900 shadow-xl animate-slide-in-right">
            <div className="flex flex-col h-full p-6">
              ...
              <div className="flex flex-col space-y-4">
                ...
                {user ? (
                  <Button variant="outline" className="w-full justify-center mt-6" onClick={async () => { await logout(); setMobileMenuOpen(false); navigate('/'); }}>Sign Out</Button>
                ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="text-center md:text-right">
              <p className="text-xs text-slate-400">&copy; 2025 Dwelgo.ng, Inc. All rights reserved.</p>
            </div>
          </div>
